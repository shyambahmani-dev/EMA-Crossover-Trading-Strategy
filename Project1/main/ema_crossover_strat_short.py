...
shortOpened = np.array([])
shortClosed = np.array([])
#"""


for ind in data1.index:

    if(data1EMA["EMA10"].loc[ind] < data1EMA["EMA50"].loc[ind] and numShort == 0 and data1DMAslope["DMA25"].rolling(3).mean().loc[ind] < 0):

        numShort = (currCash)/data1["Typical"].loc[ind]
        shortPrice = data1["Typical"].loc[ind]

        currCash += numShort*(data1["Typical"].loc[ind])
        shortOpened = np.append(shortOpened, ind)


    elif( numShort > 0 and (data1EMA["EMA3"].loc[ind] > data1EMA["EMA10"].loc[ind] ) ):
        
        currCash -= numShort*data1["Typical"].loc[ind]
        numShort = 0
        
        shortClosed = np.append(shortClosed, ind)
    
    

    portfolio.loc[ind] = currCash - numShort*data1["Typical"].loc[ind]
...
for it in shortOpened:
    ax1.axvline(it, color = 'green', alpha = 0.1)

for it in shortClosed:
